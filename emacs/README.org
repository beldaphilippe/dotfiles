...
- Install some language servers for eglot:
  #+BEGIN_SRC
    python-lsp-server          # python, add extensions for linters, ...
    nixd                       # nix
...
| C-x C-SPACE   | Go to previous cursor position |
| C-ù           | Go to matching parenthesis     |
| M-g M-g X RET | Go to line X                   |
...
- ~elp(test)a/~: contains source code for packages installed through elpa repository.
...
- [ ] org mode config
- [ ] glsl: auto formatting on save, popups on keybind to doc, errors highlighted
